...
// Matches the sport-specific imagery used on the Sports Venues and Coaches pages.
...
const TrainersCategory = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const trainersRes = await axios.get(`${API_URL}/web/PersonalTraining/fetchAll`);
  ...
  const classifyTrainers = (trainersList: Trainer[]) => {
    ...
    trainersList.forEach((t) => {
      categories.forEach((catObj) => {
        if (matchCategory(t.category, t.trainer_type, t.specializations, catObj.name)) {
          counts[catObj.id]++;
        }
      });
    });
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="hero-booking-section standard-page-hero" style={{ background: "linear-gradient(135deg, #F0FDF4 0%, #ECFDF5 100%)", paddingTop: "110px", paddingBottom: "40px", position: "relative", overflow: "hidden", borderBottom: "1px solid #E5E7EB" }}>
            <div className="hero-artwork-blend" style={{ position: "absolute", right: "-60px", top: 0, bottom: 0, width: "55%", backgroundImage: "url('/assets/img/bg/trainer-hero.png')", backgroundSize: "cover", backgroundPosition: "center", backgroundRepeat: "no-repeat", maskImage: "linear-gradient(to left, rgba(0,0,0,1) 60%, rgba(0,0,0,0) 100%)", WebkitMaskImage: "linear-gradient(to left, rgba(0,0,0,1) 60%, rgba(0,0,0,0) 100%)", opacity: 0.78 }}></div>
